...
class class_idx():
    def __init__(self, opt, source_file='./data/train_b.txt'):
        self.opt = opt
        self.pattern = '.*?\t.*?\t.*?\t.*?\t.*?\t(.*?)\t(.*?)\t(.*?)\n'
        with open(source_file) as f:
            ...
            self.idx_to_class1 = dict(
                zip([i for i in range(20)], self.class_1))
            self.idx_to_class2 = dict(
                zip([i for i in range(135)], self.class_2))
            self.idx_to_class3 = dict(
                zip([i for i in range(265)], self.class_3))
